/* =====================================================================
   Barbearia do Preto — motor de agendamento
...
(function () {
  ...
  var CHAVE = 'bp_agendamentos_demo';
  ...
  function demoBloqueios() {
    try { return JSON.parse(localStorage.getItem('bp_bloqueios_demo') || '[]'); }
    catch (e) { return []; }
  }
  ...
  function demoSemeia() {
    try { if (localStorage.getItem('bp_demo_semeado_v2')) return; } catch (e) { return; }
    ...
    demoGrava(lista);
    try { localStorage.setItem('bp_demo_semeado_v2', '1'); } catch (e) {}
  }
  ...
  try {
    var salvo = JSON.parse(localStorage.getItem('bp_cliente') || 'null');
  ...
  form.addEventListener('submit', function (ev) {
    ...
    API.criar({
      ...
      try { localStorage.setItem('bp_cliente', JSON.stringify({ nome: nome, tel: campoTel.value })); } catch (e) {}
  ...
  function mostraSucesso(a, nome) {
    ...
    var texto =
      'Olá, Barbearia do Preto! Marquei pelo site.\n\n' +
    ...
    $('#ok-ics', raiz).download = 'barbearia-do-preto-' + a.codigo + '.ics';
  ...
  function fazIcs(a) {
    ...
    var linhas = [
      'BEGIN:VCALENDAR', 'VERSION:2.0', 'PRODID:-//Barbearia do Preto//PT-BR',
      'BEGIN:VEVENT',
      'UID:' + a.codigo + '@barbeariadopreto',
      'DTSTART;TZID=America/Sao_Paulo:' + carimbo(a.dia, a.inicio),
      'DTEND;TZID=America/Sao_Paulo:'   + carimbo(a.dia, a.fim),
      'SUMMARY:' + a.servico + ' com ' + a.barbeiro + ' — Barbearia do Preto',
